...
class StrengthFragment : Fragment(R.layout.fragment_facility){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            addDetailsViewModel.CurrentFlag = "Strength"
            binding?.let {
                it.edittextFacilityItem.setOnClickListener {
                    dialog.newInstance(addDetailsViewModel, "Strength")
                    dialog.show((context as AppCompatActivity).supportFragmentManager, "Dialog")
                }
            }

            addDetailsViewModel.lststrengthdata.observe(viewLifecycleOwner){
                val adapter = StrengthRvAdapter(it!!,addDetailsViewModel)
