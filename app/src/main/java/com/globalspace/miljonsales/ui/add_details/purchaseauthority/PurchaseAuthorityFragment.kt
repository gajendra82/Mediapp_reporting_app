...
class PurchaseAuthorityFragment  : Fragment(R.layout.layout_consumption_fragment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        try {
            ...
            addDetailsViewModel.CurrentFlag = "PurchaseAuthority"
            addDetailsViewModel.lstpurchase_authoritydata.observe(viewLifecycleOwner){
                val adapter = PurchaseAuthorityAdapter(it,addDetailsViewModel)
                binding!!.rvconsumption.adapter = adapter
            }
            binding!!.tvcreate.setOnClickListener {
                try {
                    addDetailsViewModel.clearPurchaseAuthority()
                    val intent = Intent(requireActivity(), PurchaseAuthorityActivity::class.java)
